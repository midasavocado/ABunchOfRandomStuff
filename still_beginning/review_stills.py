"""Quick review without rendering the film: first / middle / last frame of every shot at 960x540, in edit order.
    ...
    python3 review_stills.py s09b s15        # just these
...
t_all = time.time()
for i, (sid, a, b, scene) in enumerate(shots):
    ...
    t = time.time()
    print("[%d/%d] %s ..." % (i + 1, len(shots), sid), end=" ", flush=True)
    ...
    print("%s %.0fs" % ("ok" if r.returncode == 0 else "FAILED", time.time() - t), flush=True)
    if r.returncode != 0:
        print(r.stderr[-1500:])
