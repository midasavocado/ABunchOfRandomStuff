...
# (no pane geometry: at night the glass would only mirror the lamp-lit room as a flat sheet at preview; the final
# look keeps the window open-feeling and clear)
...
wc = sb.light('AREA', "WindowCool", loc=(0, WY + 0.2, 1.4), target=(0, -0.5, 0.9), energy=6.0, color=(0.55, 0.65, 1.0), size=1.4)
wc.visible_camera = False
wc.visible_glossy = False
rb = sb.light('AREA', "RoomBounce", loc=(0, -1.5, 2.4), target=(0, 0, 0.5), energy=4.0, color=(1.0, 0.8, 0.6), size=1.5)
rb.visible_camera = False
rb.visible_glossy = False

# ------------------------------------------------------------------ sketchbook + the drawing (grows under the pencil)
BOOK_C = V((-0.06, 0.0, DESK_Z + 0.004))
...
def stroke(name, pts2d, width=0.0007, grow=None):
...
arc_obj = stroke("Arc", ARC, 0.0008, grow=(DRAW0, DRAW1))
...
bpy.context.view_layer.update()
# the child's own right hand in a tripod pencil grip (index pad on top, thumb on the side, pencil resting on the
# middle finger); the pencil is rigid to the hand (parented to the wrist bone), and the arm IK is iterated so the
# graphite tip traces the arc on the page
for fb, a_ in (("finger2", (28, 32, 20)), ("finger3", (40, 45, 30)), ("finger4", (65, 70, 45)), ("finger5", (70, 75, 50))):
    for j, ang in zip((1, 2, 3), a_):
        folks.pose(rig, {f"{fb}-{j}.R": (ang, 0, 0)})
folks.pose(rig, {"finger1-1.R": (10, 0, -15), "finger1-2.R": (18, 0, 0), "finger1-3.R": (22, 0, 0), "wrist.R": (-12, 0, 8)})
# first put the hand roughly at the start of the stroke, then build the pencil from the grip geometry
folks.reach(rig, "R", tip_at(F0) + V((-0.03, -0.06, 0.07)), elbow_hint=tip_at(F0) + V((0.14, -0.25, -0.05)), iters=30)
bpy.context.view_layer.update()
web = (folks.bone_world(rig, "finger1-1.R") + folks.bone_world(rig, "finger2-1.R")) / 2
grip = (folks.bone_world(rig, "finger1-3.R", tail=True) * 0.55 + folks.bone_world(rig, "finger2-3.R", tail=True) * 0.45)
PEN_D = (grip - web).normalized()
PEN_R = 0.0033
tip0 = grip + PEN_D * 0.028
back = web - PEN_D * 0.07
pencil_m = sb.painted("PencilBody", (0.75, 0.32, 0.04), rough=0.35, coat=0.3)
wood_m = sb.mat("PencilWood", (0.78, 0.6, 0.4), rough=0.6)
shaft = rocket.rod("PencilShaft", back, tip0 - PEN_D * 0.016, PEN_R, pencil_m, verts=6)
cone = rocket.rod("PencilCone", tip0 - PEN_D * 0.016, tip0, 0.0021, wood_m, verts=12)
lead = rocket.rod("PencilLead", tip0 - PEN_D * 0.004, tip0, 0.0008, graphite, verts=8)
tip_e = sb.empty("PencilTip", loc=tip0)


def attach_keep(o, mw):
    o.parent = rig
    o.parent_type = 'BONE'
    o.parent_bone = "wrist.R"
    bpy.context.view_layer.update()
    o.matrix_world = mw


bpy.context.view_layer.update()
mws = {o.name: o.matrix_world.copy() for o in (shaft, cone, lead, tip_e)}
...
for f in range(F0 - 2, F1 + 2):
    ...
    for it in range(4):
        folks.reach(rig, "R", W, elbow_hint=T + V((0.14, -0.25, -0.05)), iters=12)
        bpy.context.view_layer.update()
        e = T - tip_e.matrix_world.translation
        if e.length < 0.0015:
            break
        W = W + e
    for b in ("upperarm01.R", "lowerarm01.R"):
        rig.pose.bones[b].keyframe_insert("rotation_euler", frame=f)
    if f in (F0, F0 + 60, F1 - 1):
        print("TIPERR", f, round(e.length * 1000, 1), "mm")
